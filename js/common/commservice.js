...
angular.module('app.service', ['ngTouch']).factory('qxcard', ['$http',
		function($http) {
			return {
	...
	.factory('updateCart', ['$http',
		...
		return {
			...
			templateUrl: '/qxv3/tpl/boxinner.html',
			link: function(scope, element, attr) {
				scope.toggle = true
				scope.togglebox = function() {
					scope.toggle = !scope.toggle;
	...
	}).directive('orderinfo', ['$http', '$filter', 'orderact',
		function($http, $filter, orderact) {
			//暂时用于include静态内容，嵌入指令有问题，稍后研究
			//boxouter
			return {
				restrict: 'EA',
				...
				templateUrl: '/qxv3/tpl/orderlist.html',
				link: function(scope, element, attr) {
